# quanhau-api-docs-cli

boru-driven command-line client **and** interactive REPL for the QuanhauApiDocs
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quanhau-api-docs-cli)
make build

# 2. See usage (words, entities, env vars)
./quanhau-api-docs-cli --help

# 3. Provide credentials once, via the environment
export QUANHAU_API_DOCS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./quanhau-api-docs-cli load 1 api            # {id:1} shorthand
./quanhau-api-docs-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
QUANHAU_API_DOCS_BASE=https://api.example.com ./quanhau-api-docs-cli load 1 api

# 6. No arguments -> interactive REPL
./quanhau-api-docs-cli
quanhau-api-docs> load 1 api
quanhau-api-docs> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quanhau-api-docs-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/quanhau-api-docs-cli load 1 api
   ```
...
```sh
./quanhau-api-docs-cli load 1 api          # scalar shorthand for {id:1}
./quanhau-api-docs-cli load '{id:1}' api     # explicit match map
```
...
./quanhau-api-docs-cli load 1 api
```
...
Run with no arguments to open a REPL (prompt `quanhau-api-docs>`). Each line is
evaluated as its own boru expression:

```text
$ ./quanhau-api-docs-cli
quanhau-api-docs> load 1 api
quanhau-api-docs> /help
quanhau-api-docs> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/quanhau-api-docs-cli`. |
...
(`./quanhau-api-docs-cli <expr>`) and interactively (the REPL), and expressions compose the
