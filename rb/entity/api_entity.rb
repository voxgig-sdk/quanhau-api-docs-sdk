...
class ApiEntity
  ...
  def make
    opts = @_entopts.dup
    ApiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = QuanhauApiDocsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Api, Hash] the current Api data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = QuanhauApiDocsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Api fields)
  def match_get
  ...
  # Load a single Api.
  #
  # @param reqmatch [ApiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Api.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Api, Hash] the loaded Api; raises QuanhauApiDocsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = QuanhauApiDocsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
