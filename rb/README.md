# QuanhauApiDocs Ruby SDK



The Ruby SDK for the QuanhauApiDocs API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = QuanhauApiDocsSDK.test(testopts, sdkopts)
```
...
raise a `QuanhauApiDocsError` (a `StandardError` subclass), so wrap
...
```
rb/
├── QuanhauApiDocs_sdk.rb       -- Main SDK module
...
The main module (`QuanhauApiDocs_sdk`) exports the SDK class
