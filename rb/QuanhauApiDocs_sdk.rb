...
require_relative 'QuanhauApiDocs_types'
...
class QuanhauApiDocsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = QuanhauApiDocsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = QuanhauApiDocsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, QuanhauApiDocsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    QuanhauApiDocsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = QuanhauApiDocsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = QuanhauApiDocsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = QuanhauApiDocsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => QuanhauApiDocsError.new(
        "#{op}_allow",
        "QuanhauApiDocsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue QuanhauApiDocsError => err
    ...
    if fetched.is_a?(Hash)
      status = QuanhauApiDocsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = QuanhauApiDocsError.new(
        "graphql_error", "QuanhauApiDocsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = QuanhauApiDocsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
