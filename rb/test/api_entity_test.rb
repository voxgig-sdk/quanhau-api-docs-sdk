...
class ApiEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set QUANHAU_API_DOCS_TEST_API_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def api_basic_setup(extra)
  ...
  entid_env_raw = ENV["QUANHAU_API_DOCS_TEST_API_ENTID"]
  ...
  if env["QUANHAU_API_DOCS_TEST_LIVE"] == "TRUE"
    ...
    client = QuanhauApiDocsSDK.new(Helpers.to_map(merged_opts))
  end
