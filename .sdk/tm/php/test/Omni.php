...
//    only - a typed QuanhauApiDocsContext there would make every ctx
...
final class QuanhauApiDocsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new QuanhauApiDocsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new QuanhauApiDocsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new QuanhauApiDocsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new QuanhauApiDocsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new QuanhauApiDocsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?QuanhauApiDocsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new QuanhauApiDocsError(is_string($code) ? $code : '', $msg);
    }
