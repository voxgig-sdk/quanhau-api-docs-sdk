...
// QuanhauApiDocs SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class QuanhauApiDocsMakePoint
{
    public static function call(QuanhauApiDocsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof QuanhauApiDocsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = QuanhauApiDocsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = QuanhauApiDocsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
