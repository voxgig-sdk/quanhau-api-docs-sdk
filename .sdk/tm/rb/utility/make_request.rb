...
module QuanhauApiDocsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = QuanhauApiDocsResponse.new({})
    ...
    if fetch_err
      ...
      response = QuanhauApiDocsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = QuanhauApiDocsResponse.new(fetched)
    else
