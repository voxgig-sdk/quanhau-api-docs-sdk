...
import (
	...
	omni "github.com/voxgig-sdk/quanhau-api-docs-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.QuanhauApiDocsError); is {
				out := map[string]any{
					"name":    "QuanhauApiDocsError",
...
func omniCtx(arg any, client *sdk.QuanhauApiDocsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.QuanhauApiDocsSDK); is {
			client = live
