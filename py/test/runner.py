# QuanhauApiDocs SDK test runner
...
class QuanhauApiDocsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            QuanhauApiDocsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = QuanhauApiDocsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = QuanhauApiDocsTestRunner.getenv("QUANHAU_API_DOCS_TEST_LIVE")
        override = QuanhauApiDocsTestRunner.getenv("QUANHAU_API_DOCS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = QuanhauApiDocsTestRunner.getenv(key)
        ...
        explain = QuanhauApiDocsTestRunner.getenv("QUANHAU_API_DOCS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if QuanhauApiDocsTestRunner._test_control is not None:
            return QuanhauApiDocsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                QuanhauApiDocsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            QuanhauApiDocsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = QuanhauApiDocsTestRunner.load_test_control()
...
def load_env_local():
    QuanhauApiDocsTestRunner.load_env_local()


def env_override(m):
    return QuanhauApiDocsTestRunner.env_override(m)


def entity_data(v):
    return QuanhauApiDocsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return QuanhauApiDocsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return QuanhauApiDocsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return QuanhauApiDocsTestRunner.load_test_control()


def live_client_options():
    return QuanhauApiDocsTestRunner.live_client_options()


def live_delay_ms():
    return QuanhauApiDocsTestRunner.live_delay_ms()
