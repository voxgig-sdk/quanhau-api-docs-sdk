    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set QUANHAU_API_DOCS_TEST_API_ENTID JSON to run live")
        client = setup["client"]
...
def _api_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("QUANHAU_API_DOCS_TEST_API_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("QUANHAU_API_DOCS_TEST_LIVE") == "TRUE":
        ...
        client = QuanhauApiDocsSDK(helpers.to_map(merged_opts))

    _live = env.get("QUANHAU_API_DOCS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("QUANHAU_API_DOCS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
