...
// QuanhauApiDocs SDK retry feature
...
class QuanhauApiDocsRetryFeature extends QuanhauApiDocsBaseFeature
{
    ...
    public function init(QuanhauApiDocsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (QuanhauApiDocsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(QuanhauApiDocsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(QuanhauApiDocsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
