...
class ApiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ApiEntity($this->_client, $opts);
    }

    /**
     * @param Api|array $args Api data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = QuanhauApiDocsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Api|array The current Api data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Api fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = QuanhauApiDocsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Api fields).
     */
     ...
     * Load a single Api.
     *
     * @param ApiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ApiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Api|array The loaded Api as an assoc-array at the
     *   SDK boundary; throws QuanhauApiDocsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = QuanhauApiDocsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
