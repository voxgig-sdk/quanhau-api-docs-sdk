...
// QuanhauApiDocs SDK feature test
//
...
class FtCtrl extends QuanhauApiDocsControl
{
...
class FtClient
{
    ...
    private QuanhauApiDocsUtility $utility;

    public function __construct(QuanhauApiDocsUtility $utility, array $options)
    {
    ...
    public function get_utility(): QuanhauApiDocsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (QuanhauApiDocsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public QuanhauApiDocsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (QuanhauApiDocsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = QuanhauApiDocsConfig::make_feature($name);
    ...
    private static function buildUrl(QuanhauApiDocsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof QuanhauApiDocsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new QuanhauApiDocsResponse($fetched);
        }
    ...
    private function fail(QuanhauApiDocsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (QuanhauApiDocsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
