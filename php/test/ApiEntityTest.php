...
class ApiEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set QUANHAU_API_DOCS_TEST_API_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function api_basic_setup($extra)
{
    ...
    $client = QuanhauApiDocsSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("QUANHAU_API_DOCS_TEST_API_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["QUANHAU_API_DOCS_TEST_API_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["QUANHAU_API_DOCS_TEST_LIVE"] === "TRUE") {
        ...
        $client = new QuanhauApiDocsSDK(Helpers::to_map($merged_opts) ?? []);
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["QUANHAU_API_DOCS_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
