# quanhau-api-docs-mcp

[MCP](https://modelcontextprotocol.io) server exposing the QuanhauApiDocs SDK as
two agent tools — `quanhau-api-docs_list` and `quanhau-api-docs_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quanhau-api-docs-mcp)
make build

# 2. Provide credentials via the environment
export QUANHAU_API_DOCS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user quanhau-api-docs \
  -- /absolute/path/to/quanhau-api-docs-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./quanhau-api-docs-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// quanhau-api-docs_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// quanhau-api-docs_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quanhau-api-docs-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user quanhau-api-docs \
     -- "$PWD"/dist/*/quanhau-api-docs-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `quanhau-api-docs_list` and `quanhau-api-docs_load` tools now appear
   in new sessions. Ask the agent to *"list api using quanhau-api-docs"*
   and it calls `quanhau-api-docs_list` with `{"entity":"api"}`.
...
```sh
export QUANHAU_API_DOCS_APIKEY=sk_live_xxx            # API key
export QUANHAU_API_DOCS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `quanhau-api-docs_list` tool
...
### Call the `quanhau-api-docs_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `quanhau-api-docs_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `quanhau-api-docs_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `QUANHAU_API_DOCS_APIKEY` | API key sent with every request. |
| `QUANHAU_API_DOCS_BASE` | Optional override of the API base URL. |
...
```sh
./quanhau-api-docs-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"quanhau-api-docs_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
