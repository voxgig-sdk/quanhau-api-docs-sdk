// Command QuanhauApiDocs-mcp is an MCP server exposing the QuanhauApiDocs
// SDK as MCP tools.
//
// Two tools are registered:
//
//	QuanhauApiDocs_list  — list records of an entity
//	QuanhauApiDocs_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: QUANHAU_API_DOCS_APIKEY carries the API key and
	// QUANHAU_API_DOCS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("QUANHAU_API_DOCS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("QUANHAU_API_DOCS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewQuanhauApiDocsSDK(opts)
