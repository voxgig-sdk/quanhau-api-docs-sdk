...
local ApiEntity = {}
ApiEntity.__index = ApiEntity


function ApiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiEntity)
  self._name = "api"
  self._client = client
...
function ApiEntity:get_name()
  return self._name
end


function ApiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiEntity.new(self._client, opts)
end
...
function ApiEntity:mark_deleted()
  self._deleted = true
end


function ApiEntity:deleted()
  return true == self._deleted
end


function ApiEntity:data_set(args)
  if args ~= nil then
...
function ApiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiEntity:match_set(args)
  if args ~= nil then
...
function ApiEntity:stream(action, args, callopts)
...
---@param reqmatch ApiLoadMatch
---@param ctrl? table
---@return Api
---@return string? err
function ApiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ApiEntity:_run_op(ctx, post_done)
  local utility = self._utility
