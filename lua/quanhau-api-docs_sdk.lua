...
require("quanhau-api-docs_types")
...
local QuanhauApiDocsSDK = {}
QuanhauApiDocsSDK.__index = QuanhauApiDocsSDK
...
QuanhauApiDocsSDK._make_feature = _make_feature


function QuanhauApiDocsSDK.new(options)
  local self = setmetatable({}, QuanhauApiDocsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function QuanhauApiDocsSDK:options_map()
...
function QuanhauApiDocsSDK:get_utility()
  return Utility.copy(self._utility)
end


function QuanhauApiDocsSDK:get_root_ctx()
  return self._rootctx
end


function QuanhauApiDocsSDK:prepare(fetchargs)
...
function QuanhauApiDocsSDK:direct(fetchargs)
...
-- by passing it.
function QuanhauApiDocsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function QuanhauApiDocsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "QuanhauApiDocsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function QuanhauApiDocsSDK:Api(data)
...
function QuanhauApiDocsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = QuanhauApiDocsSDK.new(sdkopts)
  sdk.mode = "test"
