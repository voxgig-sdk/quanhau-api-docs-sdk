...
//     (\`new QuanhauApiDocsSDK(...)\` / \`QuanhauApiDocsSDK.test(...)\` become
//     \`QuanhauApiDocsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'QuanhauApiDocsSDK'
...
const TEST_SEED = {"entity":{"api":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new QuanhauApiDocsSDK(...)\` and any
// \`QuanhauApiDocsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+QuanhauApiDocsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/QuanhauApiDocsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
