...
(0, node_test_1.describe)('ApiDirect', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when QUANHAU_API_DOCS_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('QUANHAU_API_DOCS_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.QuanhauApiDocsSDK({
    ...
    (0, node_test_1.test)('direct-load-api', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-api', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.QUANHAU_API_DOCS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.QuanhauApiDocsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['QUANHAU_API_DOCS_TEST_API_ENTID'];
    ...
    const client = new __1.QuanhauApiDocsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ApiDirect.test.js.map