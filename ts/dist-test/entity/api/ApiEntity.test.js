...
(0, node_test_1.describe)('ApiEntity', async () => {
    ...
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.QuanhauApiDocsSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'api.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set QUANHAU_API_DOCS_TEST_API_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.QuanhauApiDocsSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['QUANHAU_API_DOCS_TEST_API_ENTID'];
    ...
    if (live) {
        client = new __1.QuanhauApiDocsSDK(merge([
